...
export default function Generator() {
    ...
    const defaultErrorModal = { show: false, message: 'Default Message', data: [] }
    ...
    const handleExport = () => {
        window.k.ipcRenderer.invoke('chooseOutput')
            .then(res => {
                if (res !== 'canceled') {
                    //console.log(res)
        ...
        if (files.length > 0) {
            return (
                <Button
                    ...
                    size='small'
                    onClick={() => {
                        window.k.ipcRenderer.invoke('clearImages')
    ...
    const ImagesBox = () => (
        <div style={{ height: '200px', border: '1px solid lightgrey', overflowY: 'auto', fontSize: '10px', padding: '5px' }}>
            {
                files.map((file, idx) => {
                    return (
                        <div
                        ...
                        >
                            {file.name}
                            <div>
                                <img style={{ maxWidth: '100%', maxHeight: '100%' }} src={`atom://${file.name}`} alt={file.name} />
                            </div>
                        </div>
                    )
                })
            ...
            return (
                ...
                >
                    <Box sx={style} component={Paper} >
                        <Typography id="modal-modal-title" variant="h6" component="h2">ERROR</Typography>
                        <Typography id="modal-modal-description" sx={{ mt: 2 }}>{errorModal.message}</Typography>
                        ...
                        <div style={{ textAlign: 'right' }} >
                            <Button onClick={() => setErrorModal(defaultErrorModal)} >Close</Button>
                        </div>
                    </Box>
                </Modal>
            )
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
            <Button
                ...
                onClick={async () => {
                    ...
                    window.k.ipcRenderer.invoke('chooseFolder')
                        .then(res => {
